...
import 'package:nexus_linux/app/data_dir.dart';
...
class VideoLibraryController extends GetxController {
  ...
  /// 生成单个视频的缩略图。
  ///
  /// 【与 Windows 端的差异】Windows 端优先走 `fc_native_video_thumbnail`
  /// （底层是 Windows 专有的 Media Foundation API），失败时才回退到
  /// media_kit 截图方案。该插件不支持 Linux（没有对应的 native 实现），
  /// 所以 Linux 端直接使用 media_kit 打开视频 + seek 到 10% 位置 +
  /// screenshot() 方案——也就是 Windows 端本就有的兜底方案，兼容性最好，
  /// 唯一代价是生成速度比原生取帧慢一些（需要真正启动一个播放器实例）。
  /// 由于生成过程本来就在后台队列里跑（见 [_pumpThumbQueue]，同时最多
  /// 2 个并发），不阻塞列表展示，这个速度差异对实际使用体验影响很小。
  ///
  /// 目录管理和"相对于数据目录根目录"的路径存储逻辑与 Windows 端保持一致，
  /// 不受取帧方式影响。
  Future<void> _generateThumbnail(VideoFile f) async {
    ...
    final result = await _generateWithMediaKit(f.path, outFile);

    thumbnails[f.path] = result; // 空字符串代表"生成失败"，避免反复重试
    await _saveThumbCache();
  }

  /// 缩略图目标尺寸。与 Windows 端保持一致的 160x160 正方形，网格卡片用
  /// BoxFit.cover 铺满显示（见 _VideoCard 的 Image.file），正方形素材裁剪
  /// 显示不受影响，即使 Linux 端不再走 Windows 那个"被迫正方形"的原生
  /// 插件、理论上可以生成 16:9 缩略图，这里仍保持正方形是为了让新旧缓存
  /// （用户从 Windows 端迁移数据目录过来的场景）尺寸风格保持一致。
  ...
  Future<String> _generateWithMediaKit(String videoPath, File outFile) async {
    ...
    try {
      ...
      // JPEG。保持固定的 _thumbSize，避免不同批次生成的缩略图尺寸/体积
      // 不一致。
      VideoController(
        player,
