...
class VideoPlayerController extends GetxController
  ...
  void onInit() {
    ...
    // Linux 端本地视频固定使用 MPV 后端（不再提供 VLC 选项，详见
    // app_settings_controller.dart 顶部说明）。
    //
    ...
    final Future<void> initFuture = initPlayer(config: config);

    initFuture.then((_) async {
      // 统一监听 backend 状态流（mpv 后端暴露这些流）。
  ...
  /// seek 之后，后端的 position 事件回调可能有明显延迟（甚至在某些
    ...
    return MouseRegion(
      ...
      child: Obx(() {
        ...
        return Stack(
          fit: StackFit.expand,
          children: [
            // backend.buildView() 返回 media_kit_video 的 Video。必须传
            // ctrl.playerKey（GlobalKey），让 Video 的 State 在窗口↔全屏
            // 切换时被"移动"而非销毁重建，避免 texture 表面丢失导致黑屏。
            ...
            // 整个视频画面被灰色矩形覆盖。Linux 下 media_kit_video 走原生
            // OpenGL/EGL 纹理路径，没有 ANGLE 转译层，理论上不会触发这个具体
            // bug，但"不依赖 .obs 的内容不包 Obx"本身是通用的良好实践（减少
            // 不必要的重建），这里原样保留，不额外引入风险。
            if (showControls) ...[
...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Positioned(
      ...
      // 引擎的合成路径冲突，使整个视频画面被灰色矩形覆盖。Linux 下走原生
      // OpenGL/EGL 纹理，没有 ANGLE 转译层，这个具体触发路径理论上不存在，
      // 但改用 GestureDetector 默认的 HitTestBehavior.deferToChild、给
      // Container 一个（哪怕全透明的）color 让它自己响应点击，属于更保守、
      // 无副作用的写法，这里原样保留，不需要为了"验证过 Linux 没这个问题"
      // 而改回 opaque。
          ...
          child: Row(
            children: [
              ...
              Expanded(
                ...
                // 控制条出现时整个视频画面被灰色矩形覆盖。Linux 下没有这层
                // ANGLE 转译，但 ctrl.title 本来就从不变化，直接用普通 Text
                // 也更符合语义，不需要任何响应式包裹，两端保持一致写法。
                child: Text(
                  ctrl.title,
                  style: const TextStyle(
                      color: Colors.white,
