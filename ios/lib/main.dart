...
void main() async {
  ...
  await StorageService.init(boxName: 'nexus_ios');

  // 全面屏内容延伸到状态栏/导航栏区域下方(edge-to-edge)。
  // 该模式在 Android 上意为内容延伸至状态栏/导航栏背后；在 iOS 上是
  // 无操作(iOS 没有对应的系统级"edge-to-edge"开关，安全区域交给
  // SafeArea 组件处理即可)，调用它对 iOS 无副作用。
  ...
  runApp(const NexusIosApp());
}

class NexusIosApp extends StatelessWidget {
  const NexusIosApp({super.key});
