...
class IptvPlayerController extends GetxController
  ...
  void onInit() {
    ...
    // Linux 端 IPTV 直播固定使用 MPV 后端（不再提供 VLC 选项，详见
    // app_settings_controller.dart 顶部说明）。去交错 + TS 流容错缓冲
    // 通过 applyMpvOptions 的 forceDeinterlaceFilter / forceTsResilience
    // 参数实现，弥补此前依赖 VLC 自带去交错/网络缓冲协商的部分。
  ...
  void _playStream(
    ...
    // Linux 端只有 mpv 一个后端：backend.open() 底层就是
    // player.open(Media(url))。
    backend.open(url);
    autoHideControls();

    // player.open() 会重新初始化 mpv 的内部播放状态，之前通过
    // applyMpvOptions（含 vf-add yadif）应用的滤镜链会被清空，因此
    // 每次切流后需要重新应用。
    final s = AppSettingsController.instance;
    applyMpvOptions(
      player,
  ...
  void setVolume(double v) {
    volume.value = v;
    // mpv 后端会通过 player.stream.volume 反推回来，更新到 volume.value。
    backend.setVolume(v);
  }
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      ...
      child: Stack(
        fit: StackFit.expand,
        children: [
          // backend.buildView() 返回 media_kit_video 的 Video（内部用
          // ctrl.videoController）。
          //
          // 必须传 ctrl.playerKey（GlobalKey）：分屏↔全屏切换时整个 widget
          // 子树从 _SplitPlayer 换成 _FullScreenPlayer，没有 GlobalKey 的话
          // Video 的 State 会被销毁后重建——media_kit_video 是 texture-backed
          // 渲染，旧 State 销毁会释放纹理，新 State 无法重建纹理表面，表现
          // 为全屏黑屏。用 GlobalKey 让 Flutter 把同一份 State 从旧子树
          // 位置"移动"到新位置，纹理不中断。
