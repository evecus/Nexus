...
import 'package:nexus_android/widgets/option_dialog.dart';
import 'package:nexus_android/widgets/search_dialog.dart';
...
class VideoLibraryController extends GetxController {
  ...
  /// 无条件重新扫描一次，覆盖本地缓存。[onFound] 可选，用于扫描进度弹窗
  /// 实时展示发现的文件名。
  ...
  Future<void> scanAll({void Function(String fileName)? onFound}) async {
    ...
    final videos = await LocalScanner.scanVideos(onFound: onFound);
