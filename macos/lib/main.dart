...
void main() async {
  ...
  // 用 Hive.init（而非 Hive.initFlutter）显式指定存储目录为
  // AppDataDir.root（macOS 端是 ~/Library/Application Support/nexus，
  // 详见 app/data_dir.dart 顶部注释），不依赖 path_provider 默认的用户
  // Documents 目录，也不跟 Flutter 打包自带的 data 文件夹（flutter_assets
  // 等）混在一起。
  Hive.init(AppDataDir.root.path);
  await StorageService.init(boxName: 'nexus_macos');
  ...
  runApp(const NexusMacosApp());
}

class NexusMacosApp extends StatelessWidget {
  const NexusMacosApp({super.key});
