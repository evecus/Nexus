...
/// macOS-specific settings: theme mode + seed color on top of shared base.
///
/// 与 Windows 端保持完全一致的默认后端选择逻辑（IPTV 默认 VLC，视频默认
/// MPV，均只提供 mpv/vlc 两个可选项，不展示 auto/exo）。
///
/// 【风险点】IPTV 默认选 VLC 这一条依赖 vlc_player 包在 macOS 上确实可用
/// （见 ../../pubspec.yaml 里 vlc_player 依赖处的风险说明，以及
/// ../../lib/player/vlc_backend.dart 顶部注释）。如果本地编译/运行后发现
/// vlc_player 在 macOS 上不可用，把下面 `PlayerBackendChoice.vlc` 改成
/// `PlayerBackendChoice.mpv` 即可让 IPTV 默认也走 mpv，其余逻辑不受影响。
class AppSettingsController extends BaseSettingsController {
  ...
  void onInit() {
    ...
    // IPTV 默认使用 VLC（对 HLS/TS/RTMP 兼容性最好，与 Windows 端一致）。
    ...
    // "视频播放后端"也只提供 MPV / VLC 两个选项（设置页已移除 auto），与
    // Windows 端一致。把存储里残留的 auto 迁移为 mpv（默认后端），exo 同样
    // 不展示也一并迁移为 mpv。
