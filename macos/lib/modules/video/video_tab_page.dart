...
import 'package:nexus_macos/app/data_dir.dart';
...
class VideoLibraryController extends GetxController {
  ...
  /// 原生方案：AVFoundation 直接取帧，写入 [outFile]。
  ///
  /// 【与 Windows 端的差异】该插件在 Windows 下不支持非正方形缩略图，
  /// [height] 会被忽略、固定按 [width] x [width] 输出；macOS 下插件本身
  /// 支持任意 width/height（不强制正方形）。这里仍然把 width/height 都
  /// 显式设成同一个 [_thumbSize] 值，让 macOS 端生成的缩略图与 Windows/
  /// Linux 端保持完全一致的正方形尺寸——三端共用同一套缓存哈希/命名规则，
  /// 统一成正方形可以避免"同一份数据目录在不同平台间迁移/共享时缩略图
  /// 长宽不一致"的问题，也符合"界面和操作逻辑完全和 Windows 一样"的要求。
  ///
  ...
  Future<String> _generateWithMediaKit(String videoPath, File outFile) async {
    ...
    try {
      ...
      if (bytes != null && bytes.isNotEmpty) {
        ...
        // 存相对于数据目录根目录的相对路径（而非绝对路径），这样即便整个
        // 数据目录被一起移动/备份/迁移，缓存依然有效，不需要用户重新扫描、
        // 也不会因为 containsKey 命中旧记录而卡住不重新生成（参见
        // _enqueueMissingThumbnails 的说明）。
          ...
          child: Obx(() {
            ...
            return GridView.builder(
              ...
              itemBuilder: (_, i) => Obx(() => _VideoCard(
                    file: list[i],
                    // thumbnails 里存储的是相对于数据目录的相对路径（兼容旧
