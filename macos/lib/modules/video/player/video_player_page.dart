...
import 'package:nexus_macos/app/controller/global_player_controller.dart';
import 'package:nexus_macos/player/vlc_backend.dart';
...
class VideoPlayerController extends GetxController
  ...
  void onInit() {
    ...
    // 按用户设置选择后端：vlc / mpv。auto 对本地视频返回 exo，但桌面端
    // （macOS 同 Windows/Linux）无 ExoBackend，因此 auto 一律回退到 MPV
    // 路径（即 else 分支）。
      ...
      child: Obx(() {
        ...
        return Stack(
          ...
          children: [
            ...
            // 整个视频画面被灰色矩形覆盖。macOS 下 media_kit_video 走原生
            // Metal/OpenGL 纹理路径，没有 ANGLE 转译层，理论上不会触发这个
            // 具体 bug，但"不依赖 .obs 的内容不包 Obx"本身是通用的良好实践
            // （减少不必要的重建），这里原样保留，不额外引入风险。
            if (showControls) ...[
...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Positioned(
      ...
      // 引擎的合成路径冲突，使整个视频画面被灰色矩形覆盖。macOS 下走原生
      // Metal/OpenGL 纹理，没有这层 ANGLE 转译，这个具体触发路径理论上不
      // 存在，但改用 GestureDetector 默认的 HitTestBehavior.deferToChild、
      // 给 Container 一个（哪怕全透明的）color 让它自己响应点击，属于更
      // 保守、无副作用的写法，这里原样保留，不需要为了"验证过 macOS 没这个
      // 问题"而改回 opaque。
          ...
          child: Row(
            children: [
              ...
              Expanded(
                ...
                // 控制条出现时整个视频画面被灰色矩形覆盖。macOS 下没有这层
                // ANGLE 转译，但 ctrl.title 本来就从不变化，直接用普通 Text
                // 也更符合语义，不需要任何响应式包裹，三端保持一致写法。
                child: Text(
                  ctrl.title,
                  style: const TextStyle(
                      color: Colors.white,
