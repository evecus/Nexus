    ...
    return MouseRegion(
      ...
      child: Stack(
        ...
        children: [
          ...
          // VlcPlayer/Video 的 State 会被销毁后重建。Windows/Linux 上 VLC 是
          // texture-backed，旧 State 销毁会释放纹理，新 State 无法重建纹理
          // 表面，表现为全屏黑屏；macOS 上 vlc_player 走的是原生 platform
          // view（而非纹理），理论上不受这个具体问题影响，但 State 销毁重建
          // 仍可能导致原生视图短暂闪烁/重新附加，用同一个 GlobalKey 让
          // Flutter 把同一份 State"移动"到新位置而非销毁重建，属于更保守、
          // 无副作用的写法，三端保持一致实现。
